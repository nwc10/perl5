...
The C<catch> keyword must be immediately followed by a variable declaration
in parentheses, which introduces a new variable visible to the body of the
subsequent block. (If you don't declare this variable, you will get a syntax
error.)  Inside the block this variable will contain the exception value
that was thrown by the code in the C<try> block. It is not necessary to use
the C<my> keyword to declare this variable; this is implied (similar as it
