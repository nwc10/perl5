               ...
               int flags, int action, SV *val, BIKESHED hash)
{
    ...
    if (lvalue) {
        if (!HvABH(hv)) {
            Perl_ABH_build(aTHX_ &HvABH(hv), sizeof(HE), 0);
        }
        entry = (HE *)Perl_ABH_lvalue_fetch(aTHX_ &HvABH(hv), key, klen, hash,
                                            masked_flags);
    } else {
        entry = (HE *)Perl_ABH_fetch(aTHX_ HvABH(hv), key, klen, hash,
                                     masked_flags);
        if (entry) {
                   ...
                   int k_flags, I32 d_flags, BIKESHED hash)
{
    ...
    SV *sv = (SV *)Perl_ABH_delete(aTHX_ &HvABH(hv), key, klen, hash,
            ...
            if (HeVAL(entry) != &PL_sv_placeholder) {
                if (HeVAL(entry)) {
                    if (SvREADONLY(HeVAL(entry))) {
                        HvABH(hv) = table;
                        Perl_croak(aTHX_
                                   "Attempt to delete readonly key '%" HEKf "' from a restricted hash",
                                   HeKEY_hek(entry));
                    }
...
S_hv_free_entries(pTHX_ HV *hv)
{
    ...
    Perl_ABH_Iterator iterator = Perl_ABH_end(table);
    while (1) {
        ...
        if (S_ABH_is_empty(table))
            break;

        if (Perl_ABH_at_end(table, iterator)) {
            iterator = Perl_ABH_first(table);
            assert(!Perl_ABH_at_end(table, iterator));
        }

        /* FIXME - This could be done more efficiently with a "delete last"
         * API. Once we track hash state to permit warnings for hashes being
         * changed during iteration then we could also track more state to know
         * when to restart our "iteration". */
        ...
        SV *got = (SV *)Perl_ABH_delete(aTHX_ &table,
                                        ...
                                        HeKLEN(entry),
                                        HeHASH(entry),
                                        HeKFLAGS(entry) & HV_ABH_KEY_TYPE_MASK);
        assert(got == val);
        /* This might trigger destructors - the hash needs to be consistent
         * before we fire them: */
        SvREFCNT_dec(val);
...
Perl_hv_undef_flags(pTHX_ HV *hv, U32 flags)
{
    ...
    if (HvABH(hv)) {
        Perl_ABH_demolish(aTHX_ &HvABH(hv));
        HvABH(hv) = NULL;
    }
...
Perl_hv_iternext_flags(pTHX_ HV *hv, I32 flags)
{
    ...
    if (HvIS_EMPTY(hv)) {
        /* This is needed to pass the test in t/op/hash.t:
         * "each on apparently empty hash does not leave RITER set"
         * I'm not sure if this is the best place: */
        iter->xhv_iterator = Perl_ABH_end(table);
        return NULL;
    }
    if (!(flags & HV_ITERNEXT_WANTPLACEHOLDERS) && HvUSEDKEYS(hv) == 0) {
        /* It's just placeholders. */
        /* Logically we need that reset here too: */
        iter->xhv_iterator = Perl_ABH_end(table);
        return NULL;
    }
    ...
    if (Perl_ABH_at_end(table, iter->xhv_iterator)) {
        /* Round we go again... */
...
#endif
        HE *entry = (HE *)Perl_ABH_lvalue_fetch(aTHX_ &HvABH(hv), key, len, hash, flags);
